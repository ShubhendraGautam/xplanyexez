...
from hwprobe.io import begin_io_audit, finish_io_audit
...
def _run_isolated(handler_type: type[Handler], timeout: float) -> object:
    ...
    try:
        if not parent.poll(timeout):
            ...
            if process.is_alive():
                process.kill()
                process.join(1.0)
            return _failed_report(handler, HandlerStatus.TIMED_OUT, f"handler exceeded {timeout:g}s deadline")
        message = parent.recv()
        if message[0] == "error":
            return _failed_report(handler, HandlerStatus.FAILED, f"handler failed: {message[1]}: {message[2]}")
        result = message[1]
        if isinstance(result, HandlerReport):
            result.coverage["io"] = message[2]
        return result
    except EOFError:
        return _failed_report(handler, HandlerStatus.FAILED, f"handler worker exited with code {process.exitcode}")
    finally:
        parent.close()
        process.join(1.0)
        if process.is_alive():
...
def _run_inline(handler_type: type[Handler]) -> object:
    handler = handler_type()
    begin_io_audit()
    try:
        ...
        if isinstance(result, HandlerReport):
            result.coverage["io"] = audit
        return result
    except Exception as exc:
        finish_io_audit()
        return _failed_report(handler, HandlerStatus.FAILED, f"handler failed: {type(exc).__name__}: {exc}")
...
def scan(
    ...
    *,
    ...
    for handler_type in handler_types:
        ...
        if handler.default_probe_level > policy.maximum_probe_level:
            ...
            timeout = min(policy.handler_timeout_seconds, handler.default_timeout_seconds)
            report = _run_isolated(handler_type, timeout)
        else:
            report = _run_inline(handler_type)
        report = _validate_handler_result(handler, report)
        ...
        io_coverage = report.coverage.get("io", {})
        report.coverage = {
